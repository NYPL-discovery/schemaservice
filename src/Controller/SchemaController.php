...
final class SchemaController extends Controller
{
     ...
     * @return int
     */
    protected function getLastOffset($stream = '', $partition = 0)
    {
        $configuration = new Conf();

        $consumer = new Consumer($configuration);
        $consumer->addBrokers(Config::get('MESSAGE_BROKER'));

        /**
         * @var \RdKafka\ConsumerTopic $stream
         */
        $stream = $consumer->newTopic($stream);
        $stream->consumeStart($partition, RD_KAFKA_OFFSET_END);

        $message = $stream->consume($partition, 1000);
     ...
     * )
     */
    public function createSchema($stream = '')
    {
    ...
    /**
     * @SWG\Get(
     *     path="/v0.1/current-schemas/{stream}",
     ...
     * )
     */
    public function getCurrentSchema($stream)
    {
        $schema = new Schema();
